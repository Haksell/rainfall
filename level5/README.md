...
Once again, the vulnerability is caused by a user string being used directly as the format string in `printf`. We'll try to use it to overwrite the return address of `n`, to point to `o` instead of `main`.
...
The format string is found at the 4th place on the stack.
